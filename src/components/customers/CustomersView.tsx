...
export const CustomersView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-12">
            ...
            >
              ...
              <div className="flex items-center gap-2 pt-1" onClick={(e) => e.stopPropagation()}>
                ...
                <button
                  ...
                  className="px-2.5 py-2 rounded-xl bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 text-rose-600 dark:text-rose-400 font-semibold text-xs flex items-center justify-center gap-1 transition-colors"
              ...
              <div className="flex items-center justify-between gap-2 pt-3 border-t border-slate-200 dark:border-slate-800">
                {editingCustomerId ? (
                  <button
                    type="button"
                    onClick={() => {
                      const targetCust = customers.find((c) => c.id === editingCustomerId);
                      if (targetCust) {
                        setCustomerToDelete(targetCust);
                      }
                    }}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 font-bold text-xs transition-colors"
                  >
                    <Trash2 className="w-4 h-4" />
            ...
            <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-800/50">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    if (ledgerCustomer) {
                      setCustomerToDelete(ledgerCustomer);
                    }
                  }}
                  className="px-3 py-1.5 rounded-xl bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 hover:bg-rose-100 font-semibold text-xs flex items-center gap-1.5 transition-colors"
      ...
      {customerToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 p-5 sm:p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-white">
                  Delete Customer (گاہک ڈیلیٹ کریں)
                </h3>
                <p className="text-xs text-slate-500">
            ...
            <div className="p-3.5 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700 text-xs space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-slate-900 dark:text-white">{customerToDelete.name || customerToDelete.full_name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Phone / رابطہ:</span>
                <span className="font-mono font-semibold text-slate-700 dark:text-slate-300">{customerToDelete.phone}</span>
              </div>
              {customerToDelete.cnic && (
                <div className="flex justify-between">
                  <span className="text-slate-500 font-medium">CNIC شناختی کارڈ:</span>
                  <span className="font-mono text-slate-700 dark:text-slate-300">{customerToDelete.cnic}</span>
                </div>
              )}
              <div className="flex justify-between border-t border-slate-200 dark:border-slate-700 pt-2">
                <span className="text-slate-500 font-medium">Outstanding Balance:</span>
                <span className="font-bold text-rose-600 dark:text-rose-400">
                  {formatCurrency(customerToDelete.total_outstanding || 0)}
                </span>
              </div>
            </div>

            <div className="p-3 bg-amber-50 dark:bg-amber-950/30 rounded-xl border border-amber-200 dark:border-amber-800 text-[11px] text-amber-900 dark:text-amber-200">
              <p className="font-bold">کیا آپ واقعی اس گاہک کو ڈیلیٹ کرنا چاہتے ہیں؟</p>
              <p className="text-slate-600 dark:text-slate-400 mt-0.5">
                This will delete the customer profile from your directory. All previous transaction records will be preserved safely.
              </p>
            </div>
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  if (ledgerCustomer?.id === targetId) setLedgerCustomer(null);
                  if (editingCustomerId === targetId) setModalOpen(false);
                  await deleteCustomer(targetId);
                }}
                className="px-5 py-2 rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs shadow-md transition-transform active:scale-95 flex items-center gap-1.5"
              >
                <Trash2 className="w-4 h-4" />
                <span>Delete Permanently (ڈیلیٹ کریں)</span>
              </button>
            </div>
          </div>
        </div>
